...
<table class="ipaddress_subnet table-condensed table-full">
	...
	if(!$viewer) {
		...
		if(isSubnetWriteProtected($SubnetDetails['id'])) 
		{
			...
			if(checkAdmin(false)) 
			{
				print "<a class='modIPaddr btn btn-small btn-success'		href='' rel='tooltip' title='Add new IP address' data-action='add' data-subnetId='$SubnetDetails[id]' data-id=''> 													<i class='icon-plus icon-white'></i></a> ";			# add IP
	...
	else 
	{
				...
				print "<a class='btn btn-small disabled' href='' rel='tooltip' title='Add new nested subnet (not allowed)' 	 	<i class='icon-plus-sign'></i></a> ";			# add new child subnet
