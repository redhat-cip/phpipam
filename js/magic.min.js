...
$('#popupOverlay, button.hidePopups').live('click',function(){hidePopups();});$('a.disabled, button.disabled').click(function(){return false;});$('body').on('touchstart.dropdown','.dropdown-menu',function(e){e.stopPropagation();});function randomPass(){var chars="abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890";var pass="";for(x=0;x<10;x++){i=Math.floor(Math.random()*62);pass+=chars.charAt(i);}
return pass;}
...
$('.selfDestruct').live('click',function(){$(this).parent('div').fadeOut('fast');});function createCookie(name,value,days){if(days){var date=new Date();date.setTime(date.getTime()+(days*24*60*60*1000));var expires="; expires="+date.toGMTString();}
else{var expires="";}
...
$.post("site/admin/manageSubnetEditResult.php",subnetData,function(data){$('div.manageSubnetEditResult').html(data).slideDown('fast');if(data.search("error")==-1){showSpinner();if(subnetData.search("IPaddresses")!=-1){var sectionId=$('form#editSubnetDetails input[name=sectionId]').val();var subnetId=$('form#editSubnetDetails input[name=subnetId]').val();setTimeout(function(){openLocation("subnets",sectionId,subnetId);parameter=null;},1500);}
else if(subnetData.search("ipcalc")!=-1){var sectionId=$('form#editSubnetDetails input[name=sectionId]').val();setTimeout(function(){openLocation("subnets",sectionId,"");parameter=null;},1500);}
