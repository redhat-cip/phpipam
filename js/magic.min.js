...
function hidePopup3() {$('#popupOverlay').fadeOut('fast');$('.popup').fadeOut('fast');$('body').removeClass('stop-scrolling');$('.popup_w700').css("z-index", "100");hideSpinner();location.reload();}$(document).on("click","#popupOverlay, button.hidePopups",function(){hidePopups();});$(document).on("click","button.hidePopup2",function(){hidePopup2();});$(document).on("click", "#popupOverlay, button.hidePopup3", function() { hidePopup3(); });
$('a.disabled, button.disabled').click(function(){return false;});$('body').on('touchstart.dropdown','.dropdown-menu',function(e){e.stopPropagation();});function randomPass(){var chars="abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890";var pass="";var x;var i;for(x=0;x<10;x++){i=Math.floor(Math.random()*62);pass+=chars.charAt(i);}
return pass;}
...
return null;}
if($('#IPv4top10Hosts').length>0){$.post('site/dashboard/top10_percentage.php',function(data){$('#IPv4top10').html(data);$.post('site/dashboard/top10_hosts.php',{type:"IPv4"},function(data){$('#IPv4top10Hosts').html(data);$.post('site/dashboard/top10_hosts.php',{type:"IPv6"},function(data){$('#IPv6top10Hosts').html(data);});});});}$('a.discovery').click(function() {;showSpinner();$('a.discovery').attr("disabled", 'disabled');$.post('functions/scripts/discovery.php', function(data) {$('div.popup_w700').html(data);showPopup('popup_w700');hideSpinner();});return false;});
